...
from image_stylization import image_utils, learning, model

DEFAULT_CONTENT_WEIGHTS = '{"vgg_16/conv3": 1.0}'
DEFAULT_STYLE_WEIGHTS = (
    '{"vgg_16/conv1": 1e-4, "vgg_16/conv2": 1e-4,'
    ' "vgg_16/conv3": 1e-4, "vgg_16/conv4": 1e-4}'
)


flags = tf.app.flags
flags.DEFINE_boolean("style_grid", False, "Whether to generate the style grid.")
flags.DEFINE_boolean(
    "style_crossover", False, "Whether to do a style crossover in the style grid."
)
flags.DEFINE_boolean(
    "learning_curves", True, "Whether to evaluate learning curves for all styles."
)
flags.DEFINE_integer("batch_size", 16, "Batch size")
flags.DEFINE_integer("image_size", 256, "Image size.")
flags.DEFINE_integer(
    "eval_interval_secs", 60, "Frequency, in seconds, at which evaluation is run."
)
...
flags.DEFINE_string("eval_dir", None, "Directory where the results are saved to.")
...
def main(_):
    with tf.Graph().as_default():
        ...
        if FLAGS.style_grid:
            style_row = tf.concat(
                [tf.ones([1, FLAGS.image_size, FLAGS.image_size, 3]), style_images], 0
            )
