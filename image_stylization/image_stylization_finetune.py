...
from image_stylization import image_utils, learning, model, vgg
...
flags.DEFINE_integer(
    "save_summaries_secs", 15, "Frequency at which summaries are saved, in seconds."
)
flags.DEFINE_integer(
    "save_interval_secs", 15, "Frequency at which the model is saved, in seconds."
)
flags.DEFINE_integer(
    "task",
    0,
...
def main(unused_argv=None):
    with tf.Graph().as_default():
        ...
        with tf.device(tf.train.replica_device_setter(FLAGS.ps_tasks)):
            ...
            style_weights = dict(
                (key, style_coefficient * value) for key, value in style_weights.items()
            )
            ...
            instance_norm_vars = [
                var
                for var in slim.get_variables("transformer")
