...
def imagenet_inputs(batch_size, image_size, num_readers=1, num_preprocess_threads=4):
    ...
    with tf.name_scope("batch_processing"):
        ...
        for _ in range(num_preprocess_threads):
            ...
            image_buffer, label_index, _, _ = _parse_example_proto(example_serialized)
...
def style_image_inputs(
    ...
    with tf.name_scope("style_image_processing"):
        filename_queue = tf.train.string_input_producer(
            [style_dataset_file], shuffle=False, capacity=1, name="filename_queue"
        )
        if shuffle:
            examples_queue = tf.RandomShuffleQueue(
                capacity=64,
...
def arbitrary_style_image_inputs(
    style_dataset_file,
    batch_size=None,
    image_size=None,
    center_crop=True,
...
def load_evaluation_images(image_size):
    ...
    """
    glob = os.path.join(
        tf.resource_loader.get_data_files_path(), _EVALUATION_IMAGES_GLOB
    )
    evaluation_images = tf.gfile.Glob(glob)
    if not evaluation_images:
        raise IOError("No evaluation images found")
    return tf.concat([load_image(path, image_size) for path in evaluation_images], 0)
...
def form_image_grid(input_tensor, grid_shape, image_shape, num_channels):
    ...
    input_tensor = tf.reshape(input_tensor, grid_shape + image_shape + [num_channels])
...
def _crop(image, offset_height, offset_width, crop_height, crop_width):
    ...
    with tf.control_dependencies([size_assertion]):
        image = tf.strided_slice(
            image, offsets, offsets + cropped_shape, strides=tf.ones_like(offsets)
        )
