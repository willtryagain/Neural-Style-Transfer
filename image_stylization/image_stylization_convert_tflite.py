...
from image_stylization import model, ops

flags = tf.flags
flags.DEFINE_integer("num_styles", 1, "Number of styles the model was trained on.")
...
flags.DEFINE_bool("quantize", False, "Whether to quantize the TensorFlow Lite model.")
...
def _convert_to_tflite(saved_model_dir, num_styles, image_size, quantize, output_model):
    ...
    tf.logging.info(
        "Converted to TF Lite model: %s; Size: %d KB."
...
def main(unused_argv=None):
    ...
    _convert_to_tflite(
        ...
        FLAGS.image_size,
        FLAGS.quantize,
        os.path.expanduser(FLAGS.output_model),
    )
